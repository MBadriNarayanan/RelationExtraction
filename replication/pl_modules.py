...
from utils import BartTripletHead, shift_tokens_left, extract_triplets, extract_triplets_typed
...
class BasePLModule(pl.LightningModule):
    ...
    def forward(self, inputs, labels, **kwargs) -> Dict:
        if self.hparams.label_smoothing == 0:
            ...
            labels_smoothed.masked_fill_(labels_smoothed == -100, self.config.pad_token_id)
            loss, _ = self.loss_fn(lprobs, labels_smoothed, self.hparams.label_smoothing, ignore_index=self.config.pad_token_id)
    ...
    def generate_triples(self, batch: Dict, labels: torch.Tensor) -> Tuple[List, List]:
        ...
        if dataset_name == 'conll04_typed.py':
            ...
            return ([extract_triplets_typed(rel, type_map) for rel in decoded_preds],
                    [extract_triplets_typed(rel, type_map) for rel in decoded_labels])
        elif dataset_name == 'docred_typed.py':
            type_map = {
                '<loc>': 'LOC', '<misc>': 'MISC', '<per>': 'PER',
                '<num>': 'NUM', '<time>': 'TIME', '<org>': 'ORG'
            }
            return ([extract_triplets_typed(rel, type_map) for rel in decoded_preds],
                    [extract_triplets_typed(rel, type_map) for rel in decoded_labels])
    ...
    def generate_samples(self, batch: Dict, labels: torch.Tensor) -> List[str]:
        ...
        decoded_preds = self.tokenizer.batch_decode(
            generated_tokens[relation_start==1],
            skip_special_tokens=False
        )
    ...
    def validation_step(self, batch: Dict, batch_idx: int) -> Optional[Dict]:
        ...
        forward_output['logits'] = (
            generated_tokens.detach() if self.hparams.predict_with_generate 
    ...
    def validation_epoch_end(self, outputs: List[Dict]) -> None:
        if self.hparams.relations_file:
            ...
            if dataset_name == 'conll04_typed.py':
                relations = ['killed by', 'residence', 'location', 'headquarters location', 'employer']
                match_type = "strict"
            ...
            prec_micro, recall_micro, f1_micro = score(labels, predictions, verbose=True)
